...
INSERT INTO trip_routes_next (
  id, room_id, from_item_id, from_lodging_id, to_item_id, transport_mode,
  duration_minutes, notes, version, created_at, updated_at
)
SELECT
  id, room_id, from_item_id, NULL, to_item_id, transport_mode,
  duration_minutes, notes, version, created_at, updated_at
FROM trip_routes;
...
CREATE UNIQUE INDEX idx_trip_routes_room_item_pair
  ON trip_routes (room_id, from_item_id, to_item_id)
  WHERE from_item_id IS NOT NULL;

CREATE UNIQUE INDEX idx_trip_routes_room_lodging_pair
  ON trip_routes (room_id, from_lodging_id, to_item_id)
  WHERE from_lodging_id IS NOT NULL;

CREATE INDEX idx_trip_routes_room_pair
  ON trip_routes (room_id, from_item_id, from_lodging_id, to_item_id);
